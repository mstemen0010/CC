...
void main(int argc, char *argv[])
{
	...
	mstring delimiter = ",";
        ...
        if (argc > 1)
        {
                while ((opt = getopt(argc, argv, "xr:t:")) != -1)
                {
                        switch(opt)
                        {
				case 't': 
                ...
                {
			// tag = rawTable[index].wPos(1, delimiter);
			// mergeLine = rawTable[index].wPos(2, delimiter);
			cout.flush();
				...
				{
						if (! (xtractStrayTag && templateTable[index].ccount("{") && templateTable[index].ccount("}") && ! templateTable[index].ccount(" ")))

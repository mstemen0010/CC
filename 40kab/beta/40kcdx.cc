...
fkCodex::fkCodex( mstring fileList)
{
	...
	_leaders = new table(fileName, "~", 2);
	fileName = fileList.parse(",");
	_characters = new table(fileName, "~", 2);
	fileName = fileList.parse(",");
	_squads = new table(fileName, "~", 2);
	fileName = fileList.parse(",");
	_support = new table(fileName, "~", 2);
	fileName = fileList.parse(",");
	{
...
{
	figure *newFigure = new figure(_leaders->get(name(), 2));
	return(newFigure);
}
figure *fkCodex::character(char *name)
{
	figure *newFigure = new figure(_characters->get(name, 2));
	return(newFigure);
}
squad *fkCodex::squads(char *name)
{
	squad *newSquad = new squad(_squads->get(name, 2));
	return(newSquad);
}
vehicle *fkCodex::support(char *name)
{
	vehicle *newVehicle = new vehicle(_support->get(name, 2));
	return(newVehicle);
}
...
void fkCodex::wgList(fstream &file)
{
	...
	while( readIt )
	{
		...
		else
		{
			...
			_wgLists[index] = new table(elementsFound, "~", 2);

...
void main()
{
	...
	while(! choicei)
	{
		choicei = getChoiceInta("Choose Codex to Build:", "Eldar,Imperial Guard,Orks,Space Wolfs,Tyranid,Ultra Marine", 6);
		switch(choicei)
		{
			case 1:
				strcpy(cdxtype, "el");
	...
	while (go)
	{
		choicei = getChoiceInta("Codex builder Main Menu", "Add Leader,Add Character,Add Squad,Add Support,Add Wargear List,Add Wargear Cards,Quit", 7);
...
int addSpt(char *filename)
{
	...
	int		choicei = 0;
	mstring questStr;
	mstring questStr1 = "Name,M,WS,BS,S,I,A,Ld,Number of crew,Ram Value strength,Ram value variable,Ram value constant,Ram Value Save Modifier,Number of Weapons,Notes,Point Cost";

	mstring questStr2 = "Name,Number of crew,Ram Value strength,Ram value variable,Ram value constant,Ram Value Save Modifier,Slow Speed,Combat Speed,Fast Speed,Type,Number of Weapons,Notes,Point Cost";
				

	choicei = getChoiceInta("Choose Vehicle Type:", "Walker/Dreadnought, all others", 2);
	if (choicei == 1)
		questStr = questStr1;
	else
		questStr = questStr2;
	...
	int parse = TRUE;
	fp = fopen(filename, "a+t");
	token = questStr.parse(",");
	while (parse)
	{
		if (token != "Type")
		{
        	printf("%s (q to quit):", token());
			gets(answer);
			if (strcmp(answer, "q") != 0)
			{
				if (strlen(answer) > 0)
		...
		else
		{
			...
			while (choicec != "Quit")
			{
				choicec = getChoiceNamea("Choose Vehicle Type(s):(all that are listed on the DataFax)", "Tracked,Wheeled,Bike,Skimmer,Quit", 5);
				sprintf(lineOut, "%s%s/", lineOut, choicec());
			}
			sprintf(lineOut, "%s~", lineOut);
...
int addList(char *filename)
{
	...
	while (header)
	{
		choice = getChoiceInta("Add List Menu", "Add Header,Edit Header,Quit", 3);
		if (choice == 1)
		{

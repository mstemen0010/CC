				...
				case Guard:
					...
					{	
						...
						if (! stat(optarg, &statbuf))
						{
							...
							{
								guardWhat(optarg);
								guardTable = new table(guardWhat(), 1);
							}
...
int spike::sniff(DIR * directory)
{
	...
	table *intTable = new table(2000, 1);
	time_t startTime;
	startTime = time(NULL);
        char* rawdate = (char *)ctime(&startTime);
        char date[BUFSIZ] = "";
        strncpy (date, rawdate, (strlen(rawdate) - 1));
				...
				if (direntp->d_name)
				{
					...
					if (stat(filePath, &statbuf) == 0)
					{
						if ((status = sniff(statbuf.st_uid)))
						{
							...
							char message[BUFSIZ];
							sprintf(message, "Spike: found user %s (uid: %d ) at %s", userName, statbuf.st_uid, date);
							msg(message);
							bark();
							break;
						}

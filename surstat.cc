...
main (int argc, char *argv[])
{
        ...
        for(x=0;cl && (!feof(stdin));x++)
	{
                ...
                if (strcasecmp(entries[x].name, "Month") == 0 && (strlen(entries
[x].val) > 0))
                        month = entries[x].val;
                if (strcasecmp(entries[x].name, "Year") == 0 && (strlen(entries[
x].val) > 0))
                        year = entries[x].val;
        }
	...
	if (stat(fileName, &statbuff) || statingCurrentMonth) // check to see if the file exists
	{
		...
		if (totalSurveys)
		{
			...
			cout << "<BR><H2>Help Desk Survey Statistics for: " << month << " " << year << "</H2>\n";
			outs << "<BR><H2>Help Desk Survey Statistics for: " << month << " " << year << "</H2>\n";
		...
		{
			cout << "<HTML>	\n <HEAD>\n <TITLE>\n</TITLE>\n </HEAD>\n <BODY BGCOLOR=\"#FFFFFF\">";
			cout << "<BR><BR><BR><CENTER><H2>Sorry, There is no survey data for: " << month << " " << year << "</H2></CENTER>\n";	
			cout << "</BODY>\n</HTML>\n";
			unlink(fileName());
		}
